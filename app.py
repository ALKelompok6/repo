...
realisasi_peserta_by_nama = (
	df.groupby(by=['NAMA']).sum()[['TOTAL REALISASI PESERTA']].sum()[['RENCANA PESERTA']].sort_values(by='NAMA')
)
st.dataframe(realisasi_peserta_by_nama, width=1760)
st.bar_chart(data=realisasi_peserta_by_nama, x=['NAMA'], y=['TOTAL REALISASI PESERTA', 'RENCANA PESERTA'], width=0, height=0, use_container_width=True)