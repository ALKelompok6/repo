...
df_selection.groupby(by=['NAMA BULAN']).sum()[['RENCANA PESERTA', 'TOTAL REALISASI PESERTA']].sort_values(by='NAMA BULAN')
)
...
st.line_chart(data=realisasi_jamlator_by_nama, x=['NAMA BULAN'], y=['RENCANA JAMLATOR', 'TOTAL JAMLATOR'], width=0, height=0, use_container_width=True)
