...
realisasi_peserta_by_nama = (
	df.groupby(by=['NAMA']).sum()[['RENCANA PESERTA', 'TOTAL REALISASI PESERTA']].sort_values(by='TOTAL REALISASI PESERTA')
)
st.dataframe(realisasi_peserta_by_nama)
st.bar_chart(data=realisasi_peserta_by_nama, x=['RENCANA PESERTA', 'TOTAL REALISASI PESERTA'], y=['NAMA'], width=0, height=0, use_container_width=True)